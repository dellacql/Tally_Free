...
use ul_types::{
    AccountId, Amount, Block, BlockCapacity, SignedTx, TxKind, Vote, Wire, hash_block, hex_hash,
};
...
fn handle_received_block(ledger: &Ledger, mempool: &mut Vec<SignedTx>, block: Block) -> Result<()> {
    if !ledger.is_initialized()? {
        ensure!(
            block.header.height == 0,
            "empty node must receive genesis first"
        );
...
fn publish_network_message(net: &mut ul_p2p::Net, topic: &str, msg: NetworkMessage) -> Result<()> {
