...
impl QuorumTracker {
    ...
    pub fn add_vote(&mut self, validators: &ValidatorSet, vote: Vote) -> Result<bool> {
        ...
        let local_weight = validators.weight_of(&vote.voter);
            ...
            local_weight == vote.stake_units,
            "declared stake does not match local validator set"
        );
        if vote.accept {
...
pub fn validate_proposal(ledger: &Ledger, block: &Block, validators: &ValidatorSet) -> Result<()> {
    let meta = ledger.meta()?;
        ...
        "proposal height does not extend head"
    );
    ensure!(
        block.header.parent_hash == meta.head_hash,
        "proposal parent does not match head"
    );
    ensure!(
        validators.weights.contains_key(&block.header.proposer),
