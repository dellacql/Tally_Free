...
impl ValidatorSet {
    pub fn from_state(st: &ChainState) -> Result<Self> {
        ...
        Ok(Self { total_stake: total, weights: v })
    }
    ...
    pub fn top_staker(&self) -> Option<AccountId> {
        self.weights.iter().max_by(|a,b| a.1.cmp(&b.1)).map(|(id,_)| id.clone())
    }
}

/// Return a stake-weighted vote for a block if structurally valid.
/// (Production: re-execute block and recompute state_root.)
pub fn validate_and_vote(_st: &ChainState, b: &Block, me: &AccountId, my_stake: &BigUint) -> Result<Option<Vote>> {
    let vote = Vote {
        voter: me.clone(),
