...
fn signed_transfer_moves_units_without_changing_supply() -> Result<()> {
    ...
    let block =
        ledger.build_unsigned_block(owner.clone(), 1, vec![tx], BlockCapacity::default())?;
...
fn overspend_is_rejected() -> Result<()> {
    ...
    let too_much = Amount::from_decimal_str("1.000000000000000000000000000000000000000000001")?;
